...
class AdaBoundOptimizer(optimizer.Optimizer):
  ...
  def __init__(self, learning_rate=0.001, beta1=0.9, beta2=0.999, final_lr=0.1, gamma=1e-3,
               epsilon=1e-8, use_locking=False, amsbound=False, name="Adabound"):
  ...
  def _apply_dense(self, grad, var):  
    ...
    if self._amsbound:
      ...
      v_hat_t = state_ops.assign(v_hat, math_ops.maximum(v_hat, v_t))
      v_sqrt = math_ops.sqrt(v_hat_t)
    else:
      v_hat_t = state_ops.assign(v_hat, v_hat)
      v_sqrt = math_ops.sqrt(v_t)

    # Clip the step size
    step_size = (lr_t * math_ops.sqrt(1. - beta2_power) / (1. - beta1_power)) / (v_sqrt + epsilon_t)
    lower_bound = final_lr_t * (1. - 1. / (gamma_t + 1.))
    upper_bound = final_lr_t * (1. + 1. / (gamma_t))
    step_size = m_t * clip_by_value(step_size, lower_bound, upper_bound)

    var_update = state_ops.assign_sub(var, step_size, use_locking=self._use_locking)
  ...
  def _resource_apply_dense(self, grad, var):
    ...
    beta2_power = math_ops.cast(_beta2_power, grad.dtype.base_dtype)
    ...
    v_hat = self.get_slot(var, "v_hat").handle
    if self._amsbound:
      # v_hat_t = max(v_hat_{t-1}, v_t)
      v_hat_t = state_ops.assign(v_hat, math_ops.maximum(v_hat, v_t))
      v_sqrt = math_ops.sqrt(v_hat_t)
  ...
  def _apply_sparse_shared(self, grad, var, indices, scatter_add):
    ...
    # Clip the step size
    step_size = (lr_t * math_ops.sqrt(1. - beta2_power) / (1. - beta1_power)) / (v_sqrt + epsilon_t)
    lower_bound = final_lr_t * (1. - 1. / (gamma_t * self._step + 1.))
    upper_bound = final_lr_t * (1. + 1. / (gamma_t * self._step))
    step_size = m_t * clip_by_value(step_size, lower_bound, upper_bound)
